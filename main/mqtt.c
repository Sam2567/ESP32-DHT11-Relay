                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        if (retry < MAXIMUM_RETRY) {
            ...
            ESP_LOGI(TAG_MQTT, "Retry to connect to the WIFI");
        } else {
            xEventGroupSetBits(s_wifi_event_group, WIFI_FAIL_BIT);
            xTaskCreate(smartconfig_task, "smartconfig_task", 4096, NULL, 6, NULL);
            ESP_LOGI(TAG_MQTT,"SC_task start");
        }
        ESP_LOGI(TAG_MQTT,"Connect to the WIFI fail");
        ...
        ESP_LOGI(TAG_MQTT, "Scan done");
    } else if (event_base == SC_EVENT && event_id == SC_EVENT_FOUND_CHANNEL) {
        ESP_LOGI(TAG_MQTT, "Found channel");
    } else if (event_base == SC_EVENT && event_id == SC_EVENT_GOT_SSID_PSWD) {
        ESP_LOGI(TAG_MQTT, "Got SSID and password");
        ...
        ESP_LOGI(TAG_MQTT, "SSID:%s", ssid);
        ESP_LOGI(TAG_MQTT, "PASSWORD:%s", password);
        if (evt->type == SC_TYPE_ESPTOUCH_V2) {
            ESP_ERROR_CHECK( esp_smartconfig_get_rvd_data(rvd_data, sizeof(rvd_data)) );
            ESP_LOGI(TAG_MQTT, "RVD_DATA:");
            for (int i=0; i<33; i++) {
...
void smartconfig_task(void * parm)
{
    ...
    while (1) {
        ...
        if(uxBits & CONNECTED_BIT) {
            ESP_LOGI(TAG_MQTT, "WiFi Connected to ap");
        }
        if(uxBits & ESPTOUCH_DONE_BIT) {
            ESP_LOGI(TAG_MQTT, "smartconfig over");
